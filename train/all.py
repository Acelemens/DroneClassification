...
# ========== 加入所有注意力的ResNet ==========
class ResNetWithAllAttention(ResNet):
    def __init__(self):
    ...
    def _forward_impl(self, x):
        ...
        x = self.layer1(x); x = self.ca1(x); x = self.sa1(x)
        x = self.layer2(x); x = self.ca2(x); x = self.sa2(x)
        x = self.layer3(x); x = self.ca3(x); x = self.sa3(x); x = self.selfatt3(x)
        x = self.layer4(x); x = self.ca4(x); x = self.sa4(x); x = self.selfatt4(x)
...
if __name__ == "__main__":
    ...
    model = ResNetWithAllAttention()
    pretrained = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1).state_dict()
    ...
    if best_model_state is not None:
        ...
        torch.save(best_model_state, "../models/resnet18_all_att.pth")
        print("最佳模型已保存为 models/resnet18_all_att.pth")
        print(f"最佳验证损失: {best_val_loss:.4f}")
    else:
        torch.save(model.state_dict(), "../models/resnet18_all_att.pth")
        print("模型已保存为 models/resnet18_all_att.pth")
